...
class AppGiaiTich(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Giải Tích Phân Mặt Loại 1")
    ...
    def setup_sidebar(self):
        ctk.CTkLabel(self.sidebar, text="1 Chọn loại bài toán:", font=("Arial", 13, "bold")).pack(anchor="w", pady=(10,0))
        ...
        ctk.CTkLabel(self.sidebar, text="2 Thiết lập mặt cong S:", font=("Arial", 13, "bold")).pack(anchor="w", pady=(10,0))
        ...
        ctk.CTkOptionMenu(tab_nt, variable=self.nt_loai_var, values=["Tham số hóa R(u,v)", "Descartes z=f(x,y)", "Descartes y=g(x,z)", "Descartes x=h(y,z)"]).pack(fill="x", pady=5)
        self.nt_entries = [self.create_entry(tab_nt, f"{var}(u, v):", default) for var, default in zip(['x', 'y', 'z'], ["u*cos(v)", "u*sin(v)", "2 - u**2"])]
        ...
        self.lbl_f = ctk.CTkLabel(self.frame_f, text="3️ Nhập hàm tích phân f(x,y,z):", font=("Arial", 13, "bold"))
        self.f_ent = ctk.CTkEntry(self.frame_f)
        self.f_ent.insert(0, "x**2 + y**2")
        self.cap_nhat_giao_dien_loai("Tính Diện Tích Mặt Cong")

        ctk.CTkLabel(self.sidebar, text="4️ Phương pháp giải:", font=("Arial", 13, "bold")).pack(anchor="w", pady=(10,0))
    ...
    def lay_du_lieu_mat_S(self):
        ...
        if loai == "Tham số hóa R(u,v)": return s1, s2, s3, *bounds
        ...
        try:
            ...
            if "SymPy" in pp_giai:
                ...
                if result.has(sp.Integral):
                    self.txt_result.insert("end", "SymPy không giải ra công thức chặn. Hãy chuyển sang tính nhanh xấp xỉ.\n")
                else:
